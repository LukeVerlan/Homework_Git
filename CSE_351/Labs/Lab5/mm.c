...
static block_info* search_free_list(size_t req_size) {
  ...
  free_block = FREE_LIST_HEAD; // start at head
  while (free_block != NULL) {  // move through list
    if (SIZE(free_block->size_and_tags) >= req_size) { // check size
      return free_block; // if enough size return (could cause fragmentation)
    } else {  
      free_block = free_block->next; // move to next free block
    }
  }
  return NULL;
}
...
void* mm_malloc(size_t size) {
  ...
  // Get first free block of required size
  ptr_free_block = search_free_list(req_size);

  if(ptr_free_block == NULL) {
    request_more_space(req_size); //request more space
    ptr_free_block = search_free_list(req_size); //try again
  }
  ...
  // Check remaining size after allocation
  size_t remaining_size = block_size - req_size;

  ptr_free_block->size_and_tags = 0x0; // clear header

  // need to change tag bit (bit 0) to used (guarenteed to be a 0)
  // change to allocated, previous block will be allocated. 
  ptr_free_block->size_and_tags |= TAG_USED | TAG_PRECEDING_USED;

  // Room for split?
  if(remaining_size >= MIN_BLOCK_SIZE){

    ptr_free_block->size_and_tags |= req_size; // change size to required size

    // get a pointer to header of split block 
    block_info* new_block_ptr = ((block_info*)UNSCALED_POINTER_ADD(ptr_free_block, req_size));

    // get a pointer to footer of split block 
    // No need for block_info* because next and prev wont be used
    size_t* footer_block_ptr = ((size_t*)UNSCALED_POINTER_ADD(new_block_ptr, remaining_size - WORD_SIZE));
    
    // Change header and footer information to be accurate
    new_block_ptr->size_and_tags = 0x0; // clear header
    *footer_block_ptr = 0x0; // clear footer

    //                    put the new size and flip preceding bit
    size_t new_boundary = (remaining_size) | (TAG_PRECEDING_USED);
    new_block_ptr->size_and_tags |= new_boundary; // change header
    *footer_block_ptr |= new_boundary; // change footer

    // insert the new block into the current heap
    insert_free_block(new_block_ptr);
 
  } else {

    // next block is allocated or end of heap 
    size_t* head_next_block = (size_t*)UNSCALED_POINTER_ADD(ptr_free_block, block_size);

    // no need to change footer of next block
    ptr_free_block->size_and_tags |= block_size; // full size 
    *head_next_block |= TAG_PRECEDING_USED; // ensure following block has preceeding
  }

  //return pointer to allocated block payload
...
void mm_free(void* ptr) {
  ...
  // Get the block we want to free
  block_to_free = (block_info*)UNSCALED_POINTER_SUB(ptr, WORD_SIZE);

  // free the block (will always flip used bit to be a 0)
  block_to_free->size_and_tags ^= TAG_USED;

  // get size of payload
  payload_size = SIZE(block_to_free->size_and_tags) - WORD_SIZE;

  //adjust footer of freed block
  size_t* foot_ptr = (size_t*)UNSCALED_POINTER_ADD(block_to_free, payload_size);

  *foot_ptr = block_to_free->size_and_tags; // change the footer

  // get following header
  size_t* following_block = (size_t*)UNSCALED_POINTER_ADD(block_to_free, payload_size + WORD_SIZE);

  // update following header preceding bit (was a 1, flip to 0)
  *following_block ^= TAG_PRECEDING_USED;

  // get following used bit
  size_t following_used_bit = *following_block & TAG_USED;

  //check if footer needs update too
  if(!following_used_bit){

    // get block size of following block
    size_t following_block_size = SIZE(*following_block);

    // jump to footer
    size_t* following_foot_ptr = (size_t*)UNSCALED_POINTER_ADD(following_block, following_block_size);

    // flip preceding 
    *following_foot_ptr ^= TAG_PRECEDING_USED;
  }

  // header, footer, and following tag updated
  insert_free_block(block_to_free);

  // find if there is a preceding block
  size_t preceding_bit = block_to_free->size_and_tags & TAG_PRECEDING_USED;

  // check if either neighbor block is free, only coalesce if needed
  if(!preceding_bit || !following_used_bit) coalesce_free_block(block_to_free);
