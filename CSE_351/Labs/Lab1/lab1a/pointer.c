...
int int_size() {
  ...
  /* Computes the size of an integer */

  //point to same addr in memory
  int_ptr1 = int_array;
  int_ptr2 = int_array;

  // Move the pointer over by 1 integer length
  int_ptr1 += 1; 
  
  // Cast to a char* to change the element size to 1 byte
  // This makes it return the number of bytes between the addrs instead 
  // of the number of ints 
  return (char*)int_ptr1 - (char*)int_ptr2;
}
...
int double_size() {
  ...
  /* Computes the size of a double 
     This follows the same logic as the int size above
  */

  doub_ptr1 = doub_array;
  doub_ptr2 = doub_array;
  doub_ptr1 += 1;

  return (char*)doub_ptr1 - (char*)doub_ptr2;
}
...
int pointer_size() {
  ...
  /* Compute the length of a pointer */
  /* Follows the same logic as the above functions */

  // point to the same address
  ptr_ptr1 = ptr_array;
  ptr_ptr2 = ptr_array;

  ptr_ptr1 += 1; // move 1 double* in length (The length of a pointer)
  // return the difference in bytes between these two pointers. 
  return (char*)ptr_ptr1 - (char*)ptr_ptr2;
}
...
void swap_ints(int* ptr1, int* ptr2) {
  ...
  int temp = *ptr1; // Store the value that ptr1 points to 
  *ptr1 = *ptr2;    // Change ptr1 to point to the same thing as ptr2
  *ptr2 = temp;     // ptr2 points to what ptr1 used to point to 
}
...
int change_value() {
  ...
  /* Changes the value of index 5 to be 351 */
  ...
  int_ptr1 += 5; // Move to the 5th index of the array
  *int_ptr1 = 0xFF + 0x60; // Add 91 to 255

  return *int_ptr1;  // change this line so you don't need []
}
...
int within_same_block(int* ptr1, int* ptr2) {
  /* Determines if two pointers are in the same 64 Bytes of mem */
  
  // Cant shift pointers because the compiler gets mad so instead
  // convert the ptrs to an equivalent length data type that can be shifted using
  // binary operators
  unsigned long ptr1_addr = (unsigned long)ptr1;
  unsigned long ptr2_addr = (unsigned long)ptr2;

  // By shifting addresses the pointers point to by 6, we are effectively integer dividing those addresses
  // by 64, therefore if these are equal they are in the same 64 blocks of memory 
  // (0x20 // 64 = 0x20 >> 6 == 0) (0x50 // 64 = 0x50 >> 6 == 1) etc.
  return ptr1_addr >> 6 == ptr2_addr >> 6;
}
...
int within_array(int* int_array, int size, int* ptr) {
  ...
  /* Determines if a given pointer is within an array */

  int* index_after_array = int_array + size; 

  // need to cast to char* otherwise the first 3 indexes before the array wont be a negative number
  unsigned long lower = (unsigned long)((char*)ptr - (char*)int_array);      // Find difference in bytes 

  // The last index of the array is the index after the array - one address back
  // the line below returns the difference in bytes between the two, this should be a positive becasue 
  // the last index should be 0. if ptr > last index of the array the msb will be 1 (neg number)

  //                          need to cast to a char* to move back only 1 addr (end of arr)
  unsigned long upper = (unsigned long)((char*)(((char*)index_after_array - 1)) - (char*)ptr);  

  // if either msb is one, the ptr is out of range
  int lower_msb = lower >> 63;
  int upper_msb = upper >> 63; 

  // if these are unequal, the pointer is either greater than the index at the end of the array 
  // or less than the index at the start of the array
  return lower_msb == upper_msb;
}
...
int string_length(char* s) {
  ...
  char val = *s;    // Grab first byte of string
  while(val != '\0') {  //check if the char is a null terminator
    len++;              //increment the length
    s++;                //increment the pointer to the next char
    val = *s;           //change val to the dereference of the pointer
  }
  return len;
}
...
int endian_experiment(int* ptr) {
  ...
  /* Rewrites an int to be 351351 on a little endian machine */

  char* byte_ptr;
  byte_ptr = (char*)ptr; // point to the first addr of the int
...
void selection_sort(int arr[], int arr_length) {
  ...
  /* Sorts a list from smallest to biggest */
