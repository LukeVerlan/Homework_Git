...
int get_block_size(void) {
  ...
  int block_size = 1; 
  while(1){
    //first miss will be at the start of a new block
    if(!access_cache(block_size)) return block_size;
    block_size *= 2;
  }
  return -1;
}
...
int get_cache_size(int block_size) {
  ...
  while(1){
    flush_cache();
    access_cache(0);
    for(int i = block_size; i <= max * block_size; i += block_size) access_cache(i);
    //check if evicted, if evicted cache size was maxed
    if(!access_cache(0)) return max * block_size;
    max *= 2;
  }
  return -1;
}
...
int get_cache_assoc(int cache_size) {
  ...
  while(1){
    ...
    for(int i = 1; i <= assoc; i++) {
      // Start of each cache image matches to set 0
      access_cache(i * cache_size);
    }
    if(!access_cache(0)) return assoc; 
    assoc *= 2; 
  }
  return -1;
}
